  ...
  <div>
      ...
      <!-- 代收  毛利 折线图 -->
        <yd-cell-group class="mb10 mt10 box">
            <yd-cell-item>
                <span slot="left" >收费折线图</span>
            </yd-cell-item>
            <yd-cell-item class="not"   @click.native="searchShows=true">
                <span slot="left" >收费总额:{{total}}</span>
                <span slot="right" class="flex-5 ellipsis">收费项目:{{types.filter(res=>res.pk_id===currenttype)[0].name}}</span>
            </yd-cell-item>
             <div id="profit"></div>
        </yd-cell-group>
      ...
      <!-- 收入数据结构图 -->
       <yd-cell-group class="mb10 mt10">
            <yd-cell-item>
                <span slot="left" >收费占比图</span>
            </yd-cell-item>
             <div id="income" ></div> 
             <div class="pb20">
                <div>
                    <div :class="{'pr10':i%2==0,'pl10':i%2==1,'flex-1':1 , 'font':1, 'tr':i%2==0}"  v-for="(item , i) in incomeData" :key="i" style="width:50%;float:left;"><div :style="{'background-color':color[i]}" class="color"></div > {{item.name}} : {{item.value?item.value.toFixed(2):0}} </div>            
...
//    },
  ...
  mounted () {
      ...
      if(sessionStorage.getItem('chargeSD')){
          this.searchData = JSON.parse(sessionStorage.getItem('chargeSD'))
      }else{
          try {
  ...
  methods: {
      ...
      okselect(res,key){
          if(res.pk_id){
            ...
            for(let i in this.datas){
                if(res.pk_id==this.datas[i].key){
                    for(let j in this.datas[i].detail){
                        this.series.header.push(this.datas[i].detail[j].time)
                        this.series.body.push(this.datas[i].detail[j].total)
                    }
      ...
      finance_operation(){
          ...
          api.getChargeQuery(this.searchData).then(res=>{
              this.series={
                  header:[],
                  body:[]
              }
              ...
              for(let i in res.data){
                    res.data[i].value=Number(res.data[i].value.toFixed(2))
                    if(res.data[i].key!=='all'){
                        this.incomeData.push({value:res.data[i].value,name:res.data[i].name})
                    }else{
                        this.total=res.data[i].value
                        for(let j in res.data[i].detail){
                            this.series.header.push(res.data[i].detail[j].time)
                            this.series.body.push(res.data[i].detail[j].total)
                        }
                    }
              }
              this.datas=res.data
              ...
              this.incomeDataInit()
              this.profitDataInit()
              sessionStorage.chargeSD = JSON.stringify(this.searchData)
          })
      },
      ...
      routerList(){
          ...
          console.log(this)
          this.$router.push({ name: '/count/chargeList', params: { data: JSON.stringify(this.searchData)}})
      }
