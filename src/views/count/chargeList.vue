        ...
        <div slot="list">
            ...
            <div>
                <div class="flex title"  style="border-bottom: 1px solid #eee;">
                    <span class="flex-1">上缴日期</span>
                    <span class="flex-1">金额</span>
                    <span class="flex-1">订单数</span>
                </div>
            </div>
            <div>
                <div class="flex" v-for="(item,i) in list" :key="i" @click="routerDetail(i)"  style="border-bottom: 1px solid #eee;">
                    ...
                    <span class="flex-1">{{item.total | number_3_2}}</span>
...
export default {
  ...
  methods: {
      ...
      getList(){
        ...
        this.form.limit=this.pageInfo.limit
        api.getChargeQueryList(this.form).then(res=>{
            this.list=this.list.concat(res.data.data)
            if(!res.data.current_page){
                this.$refs.infinitescrollDemo.$emit('ydui.infinitescroll.loadedDone')
            }else if(res.data.currentrespage===res.data.last_page||res.data.current_page>res.data.last_page){
                this.$refs.infinitescrollDemo.$emit('ydui.infinitescroll.loadedDone')
            }else{
    ...
    routerDetail(index){
        this.$router.push({ name: '/count/chargeDetail', params: { data: JSON.stringify({
            date:this.list[index].time,
            type:this.form.type,
            fk_org_id:this.form.fk_org_id,
            fk_hpl_id:this.form.fk_hpl_id,
            order_num:this.list[index].orderNum,
        })}})
    }
  }
